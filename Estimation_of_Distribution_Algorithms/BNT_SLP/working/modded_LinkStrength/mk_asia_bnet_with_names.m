function bnet = mk_asia_bnet_with_names(CPD_type, p, arity)

% This is the same file that comes with the BNT distribution with only
% one modification:  
% The call to mk_bnet includes the names of the nodes.  See below: 
% 
%bnet = mk_bnet(dag, ns, 'discrete', dnodes, 'names', {'Smoking', 'Bronchitis', 'LungCancer', 'VisitToAsia', 'Tuberculosis', 'CancerOrTuberculosis', 'Dyspnoea', 'XrayPositive'});
%
...
% Following line was changed to include node names.
bnet = mk_bnet(dag, ns, 'discrete', dnodes, 'names', {'Smoking', 'Bronchitis', 'LungCancer', 'VisitToAsia', 'Tuberculosis', 'CancerOrTuberculosis', 'Dyspnoea', 'XrayPositive'});
