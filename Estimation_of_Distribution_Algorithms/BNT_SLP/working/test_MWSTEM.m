...
close all;
 ...
 nbloopmax = 6;        % number of loop max in MWST-EM
...
fprintf('Pourcentage de données manquantes : %3.1f%%\n',100*DM);
...
[BT_J11, Sbest0] = learn_struct_mwst_EM(data, discrete, node_sizes, prior, nbloopmax);
    tmp=cputime-tmp;
    fprintf('\tL''algorithme MWST-EM a duré %3.2f secondes\n',tmp);
...
figure(1),  subplot(2,2,2), draw_graph(BT_J11.dag,names,carre,xx,yy); %,carre);
    ...
    tmp=cputime-tmp;
    fprintf('\tL''algorithme SEM a duré %3.2f secondes\n',tmp);
    ...
    tmp=cputime-tmp;
    fprintf('\tL''algorithme SEM+T a duré %3.2f secondes\n',tmp);
